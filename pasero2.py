...
for serotype in selected_serotypes:
    ...
    if y_choice == 1:
        line, = ax.plot(data['YearMonth'], data['Number of isolates'], linestyle=styles[i], linewidth=widths[i], color='blue')
        ax.set_ylabel('Number of isolates')
    else:
        line, = ax.plot(data['YearMonth'], data['Past two years average'], linestyle=styles[i], linewidth=widths[i], color='blue')
        ax.set_ylabel('% Change')
        ax_right = ax.twinx()
        line_right, = ax_right.plot(data['YearMonth'], data['% Change'], linestyle=styles[(i+1)%8], linewidth=widths[(i+1)%8], color='red')            
